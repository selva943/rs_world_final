...
import { Menu, X, ShoppingBasket, MessageCircle, Search, ChevronRight, Sparkles, Calendar, Zap, Package, User, ClipboardList, ShoppingCart } from 'lucide-react';
...
export function Header() {
  ...
  const primaryLinks: NavLink[] = [
    { path: '/', label: 'Home' },
    { path: '/deliverables', label: 'Marketplace' },
    { path: '/recipes', label: 'Recipes', badge: 'Popular'  },
    { path: '/subscriptions', label: 'Subscriptions'},
    { path: '/services', label: 'Services' },
  ];

  const secondaryLinks: NavLink[] = [
    { path: '/offers', label: 'Offers', icon: <Sparkles className="w-4 h-4" /> },
    { path: '/my-orders', label: 'Activity', icon: <ClipboardList className="w-4 h-4" /> },
    { path: '/contact', label: 'Support', icon: <MessageCircle className="w-4 h-4" /> },
  ];
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        {/* Center: Desktop Navigation */}
        <nav className="hidden lg:flex items-center bg-gray-50/40 backdrop-blur-xl px-4 py-1.5 rounded-full border border-gray-200/50 gap-0.5 shadow-sm">
                ...
                <AnimatePresence>
                  {showSubscriptionMenu && (
                    <motion.div
                      ...
                      className="absolute top-full left-0 mt-4 w-64 bg-white/90 backdrop-blur-2xl rounded-2xl shadow-2xl shadow-emerald-900/10 border border-gray-100 overflow-hidden p-3"
                    >
          ...
          <div className="w-px h-4 bg-gray-200 mx-2 hidden xl:block" />

          {/* More menu for larger screens */}
          <div className="hidden xl:flex items-center gap-1">
             {secondaryLinks.slice(0, 2).map(link => (
                <Link 
                  ...
                  to={link.path}
                  className={cn(
                    "p-2 text-gray-400 hover:text-pb-green-deep hover:bg-white/60 rounded-full transition-all",
                    isActive(link.path) && "text-pb-green-deep bg-white"
                  )}
                  title={link.label}
                >
                   {link.icon}
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <Link to="/checkout" className="relative">
            <Button variant="ghost" className="w-10 h-10 rounded-full p-0 flex items-center justify-center text-pb-green-deep bg-emerald-50 hover:bg-emerald-100">
          ...
          {/* Mobile Menu Button */}
           <button
            className="lg:hidden w-10 h-10 flex items-center justify-center rounded-xl bg-gray-50 text-pb-green-deep border border-gray-100"
